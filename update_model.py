...
def annual_series(facts,tags,preferred_unit=None):
    """Merge alternate tags by FY and keep the latest filed annual value for each year."""
    if not facts: return {}
...
def put_company(wb,ticker,info):
    ...
    vals={
        "B4":ticker,
        "B5":info.get("longName") or ticker,
        "B6":info.get("sector"),
        "B7":info.get("industry"),
        "B8":p,
        "B10":info.get("marketCap")/1e9 if info.get("marketCap") else None,
...
def _template_history_fallback(wb):
    ...
    for c in range(2,8):
        ...
        out[year]={
            "revenue":bn_to_raw(ws.cell(4,c).value),"op":bn_to_raw(ws.cell(9,c).value),
            "capex":bn_to_raw(ws.cell(15,c).value),"depr":bn_to_raw(ws.cell(18,c).value),
            "ocf":bn_to_raw(ws.cell(14,c).value),"ni":bn_to_raw(ws.cell(11,c).value),
...
def update_scenarios(wb,hist,info):
    ...
    hc=max(.02,min(.20,float(hc))); opm=max(.10,min(.45,float(opm))); cap=max(.05,min(.35,float(cap))); dep=max(.01,min(.10,float(dep)))
    ...
    margins=[fade(max(.15,opm-.03),max(.15,opm-.04)),fade(opm,min(.40,opm+.01)),fade(min(.45,opm+.01),min(.45,opm+.04))]
...
def get_ticker():
    ...
    if " " in raw or ".py" in raw.lower():
        ...
        candidates=[p.upper() for p in parts if re.fullmatch(r"[A-Za-z0-9.\-]{1,10}",p) and not p.lower().endswith(".py") and p.lower() not in {"python","py"}]
...
def main():
    ...
    try: facts=company_facts(ticker)
        ...
        print(f"Warning: SEC Company Facts unavailable: {exc}")
        facts=None
    hist=build_history(ticker,facts)

    wb=load_workbook(TEMPLATE,data_only=False)
    put_company(wb,ticker,info)
    put_history(wb,hist)
    update_scenarios(wb,hist,info)
    ensure_stress_test(wb)
    update_peers(wb,ticker)
    ...
    out=OUTDIR/f"{ticker}_Equity_Research_{datetime.now():%Y%m%d_%H%M%S}.xlsx"
    wb.save(out)
    print("Saved:",out)
