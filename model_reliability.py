...
- forward scenario D&A uses the latest disclosed D&A intensity when available;
...
def right_align_history(wb):
    ...
    if len(years)<6:
        h["A24"]="Coverage note"; h["B24"]=f"{len(years)} comparable annual periods are available ({years[0]}–{years[-1]}). They are right-aligned so the latest actual is in column G; missing history is not invented."
...
def repair_history_dashboards(wb):
    rev_cagr,y0,y1,n=_available_cagr(wb,4); fcf_cagr,fy0,fy1,fn=_positive_cagr(wb,16)
    ...
    if "Advanced Analytics" in wb.sheetnames:
        ...
        for r in range(40,min(ws.max_row,55)+1):
            if str(ws.cell(r,1).value or "")=="Growth":
                ...
                # Recompute composite if the standard seven score rows are present.
...
def repair_data_quality(wb,ticker):
    ...
    # Replace old fixed-six-year tests when present.
    fixed=rows.get("Six annual periods") or rows.get("Annual history coverage")
    if fixed:
        ws.cell(fixed,1,"Annual history coverage"); status="PASS" if len(years)>=3 else ("REVIEW" if len(years)==2 else "FAIL"); ws.cell(fixed,2,status); ws.cell(fixed,3,f"{len(years)} comparable annual periods: {years}" if years else "No annual periods"); ws.cell(fixed,4,"Newer/spun companies are not failed solely because six historical years are unavailable."); ws.cell(fixed,2).fill=_fill(PALE_GREEN if status=="PASS" else GOLD)
...
def prepare_model_reliability(wb,ticker):
    years=right_align_history(wb); sync_scenario_d_and_a(wb); return years
