...
Those calculations are labelled and never presented as independently sourced estimates.
"""
...
# Discovery seeds only. Every symbol is revalidated unless it is an explicitly mapped
# business-model peer with independent market-structure evidence.
SECTOR_FALLBACK={
...
# Broader strategic comparison seeds are used only after direct/exact-industry peers.
STRATEGIC_PEERS={
...
def _enrich_info(symbol,raw=None):
    ...
    if info.get("returnOnEquity") in (None,"") and net_income is not None:
        avg_equity=None
        if equity0 is not None and equity1 is not None: avg_equity=(equity0+equity1)/2
        elif equity0 is not None: avg_equity=equity0
    ...
    if info.get("totalRevenue") in (None,"") and revenue is not None:
        info["totalRevenue"]=revenue

    ev=_num(info.get("enterpriseValue"))
    mc=_num(info.get("marketCap")); debt=_num(info.get("totalDebt")); cash=_num(info.get("totalCash"))
    ...
    if info.get("forwardPE") in (None,""):
        ...
        if feps in (None,0):
            try:
                ...
                if est is not None and not getattr(est,"empty",True) and "0y" in est.index:
                    feps=_num(est.loc["0y"].get("avg"))
            except Exception:
                pass
...
def _info(symbol):
    ...
    try:
        ...
        raw={}
    return _enrich_info(symbol,raw)
...
def _discover_candidates(ticker,sector,industry,sector_key,industry_key,region):
    ...
    out.extend(STRATEGIC_PEERS.get(str(ticker).upper(),[]))
...
def _rank(target_ticker,target_sector,target_industry,target_mc,symbol,info):
    ...
    if symbol in preferred:
        # Direct operating peers outrank accounting-industry classifications. Independent
        # market-share mapping provides an additional validation signal for these cases.
        idx=preferred.index(symbol)
        return -100+idx*.05
...
def select_dynamic_peers(wb,ticker,count=9):
    ...
    for symbol in _discover_candidates(ticker,sector,industry,sector_key,industry_key,region):
        ...
        info,notes=_info(symbol); info["_metric_notes"]=notes
        score=_rank(ticker,sector,industry,target_mc,symbol,info)
        if score is not None:
            ranked.append((score,symbol,info))
        elif symbol in preferred_peer_symbols(ticker,industry) and market_share_record(symbol):
            # Keep a direct peer even if public classification metadata is incomplete.
            ranked.append((-90,symbol,info))
...
def _metric_row(symbol,info,sector,industry,method):
    notes=info.get("_metric_notes") or []
    source_note="Live Yahoo fields" if not notes else "Live Yahoo + calculated fallback: "+"; ".join(notes)
    return [
        info.get("longName") or info.get("shortName") or symbol,symbol,info.get("forwardPE"),info.get("enterpriseToRevenue"),
        ...
        None,None,None,method,_coverage(info),source_note,
    ]
...
def _unmerge_peer_area(ws):
    for merged in list(ws.merged_cells.ranges):
        if merged.min_row<=30 and merged.max_row>=1 and merged.min_col<=18 and merged.max_col>=1:
...
def ensure_dynamic_peer_comps(wb,ticker,count=9):
    ...
    label=industry if industry!="Unknown" else sector
    ws["A1"]=f"{label} — Expanded Peer Comps"; ws["A1"].font=Font(bold=True,color=WHITE,size=18)
    ws["A2"]=("Direct business-model peers are prioritized, then exact-industry and strategic same-sector comps. "
              "Missing public metrics are calculated from statements when possible. Industry market share and peer-set market-cap weight remain separate.")
    ...
    if sector!="Unknown":
        ...
        for symbol,info in peers:
            all_rows.append((symbol,info,_peer_type(ticker,industry,symbol,info)))
        ...
        for r,(symbol,info,method) in enumerate(all_rows,4):
            ...
            row[12]=share.get("share") if share else None
        ...
        ws["A5"]="REVIEW — target sector could not be resolved from live ticker metadata. No peer set was generated."
    ...
    if sector!="Unknown" and not peers:
        ws["A5"]="REVIEW — no validated peers were returned. Stale template peers were removed rather than reused."
        ws["A5"].fill=_fill(GOLD); ws["A5"].font=Font(color=INPUT_BLUE,bold=True); ws["A5"].alignment=Alignment(wrap_text=True)

    ws.cell(method_row,1,"Method")
    ws.cell(method_row,2,"Expanded to target + up to 9 peers. Direct operating peers are prioritized. Calculated fallback metrics use Yahoo Finance statements/price data and are explicitly labelled; unsupported fields remain blank.")
    ws.cell(method_row,2).alignment=Alignment(wrap_text=True)
