...
CONFIGS={
    ...
    # MetLife reorganized in Q4 2025; current 2025 10-K reportable segments.
...
def _row_metric(text):
    low=text.lower()
    if any(k in low for k in ("operating income","operating loss","income (loss) from operations","segment profit","segment income","profit (loss)","adjusted operating income","adjusted earnings","segment ebitda","adjusted ebitda","operating profit")):
        return "op"
    if any(k in low for k in ("net sales","revenue","revenues","external sales","sales to external customers")):
        return "revenue"
    return None
...
def _find_label(values,labels):
    ...
    first=cells[0].lower(); exact=[lab for lab in labels if first==lab.lower()]
...
def _table_segment_candidates(tables):
    ...
    for rows in tables:
        ...
        for pos,vals in enumerate(rows):
            ...
            # Numeric segment rows remain the strongest signal.
            if len(nums)>=2 and _metric_context(rows,pos) in ("revenue","op"):
                candidates.setdefault(_normalize_label_text(name),0); candidates[_normalize_label_text(name)]+=3
                continue
            # Text-only heading followed by segment economics is also useful.
...
def _parse_list(text):
    s=_clean_text(text).replace("“",'"').replace("”",'"')
    s=re.split(r"\.\s+(?:In addition|See |These |The Company|We )",s,maxsplit=1)[0]
    if ";" in s:
        raw=[x.strip() for x in s.split(";")]
    else:
        # Conservative fallback for simple comma lists. Avoid splitting geographic names with many commas.
        raw=[x.strip() for x in re.split(r",\s+(?=[A-Z][A-Za-z0-9&' -]{1,45}(?:,| and |$))",s)]
    out=[]
    for x in raw:
        x=re.sub(r"^(?:and|the)\s+","",x,flags=re.I).strip(" .:,-")
        x=re.sub(r"\s+\(?[\"']?([A-Z]{2,8})[\"']?\)?$",lambda m:" ("+m.group(1)+")" if "(" in x else m.group(0),x)
...
def _discover_segments(tables,raw_html=None):
    table_names=_table_segment_candidates(tables); narrative=_narrative_segment_candidates(raw_html)
    out=[]
    # Narrative list generally expresses the complete reportable-segment universe.
...
def _years(wb):
    h=wb["Historical Financials"]; ys=[h.cell(3,c).value for c in range(2,8)]
    ys=[int(x) for x in ys if isinstance(x,(int,float))]
...
def _write_sheet(wb,ticker,years,segments,business,url,auto_status):
    ...
    ws["A3"]=(f"Standardized segment schema. Status: {auto_status}. All reportable segment names discovered in the filing are retained even when reliable numbers are unavailable. "
              "Blank yellow financial cells mean disclosed economics were not reliably extracted; they are not estimates.")
    ...
    for i in range(max_rows):
        ...
        if item:
            ...
            for c in range(1,10):
                ws.cell(r,c).fill=_fill(GOLD); ws.cell(r,c).font=Font(color=INPUT_BLUE); ws.cell(r,c).number_format=FMT_BN if c>=2 else "General"
            for c in (5,6,10,11,12,13,14): ws.cell(r,c).number_format=FMT_PCT
    ...
    for i in range(bmax):
        ...
        if item:
            ...
            ws.cell(r,8,"SEC 10-K"); ws.cell(r,8).font=Font(color=LINK_GREEN)
            for c in (5,6,7): ws.cell(r,c).number_format=FMT_PCT
        else:
            for c in range(1,9):
                ws.cell(r,c).fill=_fill(GOLD); ws.cell(r,c).font=Font(color=INPUT_BLUE); ws.cell(r,c).number_format=FMT_BN if c in (2,3,4) else "General"
            for c in (5,6,7): ws.cell(r,c).number_format=FMT_PCT
    ...
    ws.column_dimensions["O"].width=27; ws.column_dimensions["P"].width=34; ws.column_dimensions["H"].width=20
...
def ensure_segment_analysis_v2(wb,ticker,headers):
    ...
    configured=list(cfg.get("segments",[])); discovered=_discover_segments(tables,raw_html)
    # Preserve configured names first, then any additional filing-discovered names.
    seg_labels=[]
    for lab in configured+discovered:
        if lab and lab not in seg_labels: seg_labels.append(lab)
    bus_labels=list(dict.fromkeys(cfg.get("business",[])))
    ...
    for lab in seg_labels:
        ...
        # Always retain the label, even if both arrays are blank.
        segments.append((lab,rev,op))

    business=[]
    for lab in bus_labels:
        rev=raw_rev.get(lab); vals=[_num(x)*scale if _num(x) is not None else None for x in rev] if rev else [None,None,None]
        business.append((lab,vals))

    numeric_segments=sum(any(_num(v) is not None for v in (rev+op)) for _,rev,op in segments)
    narrative_count=len(discovered)
    if seg_labels:
        status=f"AUTO/BEST EFFORT — {len(seg_labels)} segment name(s) retained, {numeric_segments} with extracted financials; filing discovery={narrative_count}; parser={parser_status}"
    elif business:
        status=f"AUTO/BEST EFFORT — no reportable segment names identified, {len(business)} business/revenue group(s) retained; parser={parser_status}"
    else:
        status=f"MANUAL REQUIRED — no reliable segment names or tables extracted; parser={parser_status}"
