...
def _row_metric(text):
    low=text.lower()
    if any(k in low for k in ("operating income","operating loss","income (loss) from operations","segment profit","segment income","profit (loss)")):
        return "op"
    if any(k in low for k in ("net sales","revenue","revenues","external sales")):
        return "revenue"
    return None
...
def _company_revenue(wb):
    return _num(wb["Historical Financials"]["G4"].value) if "Historical Financials" in wb.sheetnames else None
...
def _write_sheet(wb,ticker,years,segments,business,url,auto_status):
    ...
    ws["A3"]=f"Standardized segment schema. Status: {auto_status}. Only issuer-disclosed data should be entered; yellow cells are manual inputs when automatic extraction is incomplete."
    ...
    ws.cell(source_row+1,1,"SEC 10-K Source"); ws.cell(source_row+1,2,url or "")
    ws.cell(source_row+2,1,"Extraction Status"); ws.cell(source_row+2,2,auto_status)
    ws.cell(source_row+3,1,"Important"); ws.cell(source_row+3,2,"Segment disclosure differs by issuer. If operating income is not disclosed by segment, profitability cells remain blank rather than estimated.")
