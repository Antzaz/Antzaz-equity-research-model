"""Build a robust native Excel Analysis Charts sheet.

Key design rules:
- Never depend on openpyxl evaluating Excel formulas. Helper tables link to source
...
The module also installs a type-safe repair hook for visualization_v2 so the visual
layer does not divide formula strings in Python.
"""
...
def _safe_visual_repair_advanced(wb, ticker):
    """Display-only repair; never perform Python math on Excel formula strings."""
        ...
        for r in range(7,13):
            ...
            if year in eps:
                ...
                if _is_num(price):
                    ...
                    ws.cell(r,4,pe); ws.cell(r,4).number_format=FMT_MULT; pe_values.append(pe)
                else:
                    ws.cell(r,4,f'=IFERROR(B{r}/C{r},"")'); ws.cell(r,4).number_format=FMT_MULT
        if pe_values:
            ws["G7"]=min(pe_values); ws["G8"]=statistics.median(pe_values); ws["G9"]=max(pe_values)
            for a in ("G7","G8","G9"): ws[a].number_format=FMT_MULT
    for r in range(7,15): ws.cell(r,9).number_format="yyyy-mm-dd"
    # advanced_analytics_v2 already writes the scorecard numerically. Do not rebuild it.


_visualization_v2._repair_advanced = _safe_visual_repair_advanced


def _band(ws, start_col, end_col, row, title):
    """Styled section band without merging cells."""
...
def _card(ws, start_col, end_col, title_row, value_row, title, formula, fmt):
    for c in range(start_col,end_col+1):
    ...
    ws.cell(title_row,start_col,title); ws.cell(title_row,start_col).alignment=Alignment(horizontal="center")
    ws.cell(value_row,start_col,formula); ws.cell(value_row,start_col).font=Font(bold=True,color=GREEN,size=15)
    ws.cell(value_row,start_col).alignment=Alignment(horizontal="center",vertical="center"); ws.cell(value_row,start_col).number_format=fmt
...
def _business_source_rows(wb):
    ...
    for c in range(1,min(s.max_column,15)+1):
        text=str(s.cell(header,c).value or "").strip().lower()
        if text in {"latest","2025"} or "latest revenue" in text: latest=c; break
    out=[]
    for rr in range(header+1,min(s.max_row,header+20)+1):
        name=s.cell(rr,1).value
        if name in (None,""):
            if out: break
            continue
        out.append((rr,latest,str(name)))
    return out[:10]


def _margin_source_rows(wb):
    if "Segment Analysis" not in wb.sheetnames: return [],[]
    s=wb["Segment Analysis"]; r=_find_row(s,"Reported Operating Segments")
    if not r: return [],[]
    header=r+1; margin_cols=[]
    for c in range(1,min(s.max_column,18)+1):
        text=str(s.cell(header,c).value or "")
        if "Margin" in text and "Δ" not in text: margin_cols.append(c)
    if len(margin_cols)<2: return [],[]
    margin_cols=margin_cols[-3:]
    rows=[]
...
def ensure_analysis_charts(wb,ticker):
    ...
    # No merges anywhere on this sheet.
    for c in range(1,17): ws.column_dimensions[get_column_letter(c)].width=11
    for c in range(24,55): ws.column_dimensions[get_column_letter(c)].hidden=True
    ws.freeze_panes="A5"
    ...
    for out_r,src_r in enumerate(range(22,27),3):
        ws.cell(out_r,50,_xlref("DCF",f"A{src_r}")); ws.cell(out_r,50).number_format=FMT_PCT
        for out_c,src_c in enumerate(range(2,7),51):
    ...
    if margin_end>=4:
        ...
        ch.add_data(Reference(ws,min_col=43,max_col=45,min_row=2,max_row=margin_end),titles_from_data=True); ch.set_categories(Reference(ws,min_col=42,min_row=3,max_row=margin_end)); ch.y_axis.numFmt="0%"; ws.add_chart(ch,"I53")
    ...
    _band(ws,9,16,73,"DCF Sensitivity")
    ws["I75"]="WACC / TGR"
    for out_c,helper_c in enumerate(range(51,56),10): ws.cell(75,out_c,_xlref("Analysis Charts",f"{get_column_letter(helper_c)}2")); ws.cell(75,out_c).number_format=FMT_PCT
    for out_r,helper_r in enumerate(range(3,8),76):
        ws.cell(out_r,9,_xlref("Analysis Charts",f"AX{helper_r}")); ws.cell(out_r,9).number_format=FMT_PCT
        for out_c,helper_c in enumerate(range(51,56),10):
            ws.cell(out_r,out_c,_xlref("Analysis Charts",f"{get_column_letter(helper_c)}{helper_r}")); ws.cell(out_r,out_c).number_format=FMT_PRICE
    _header(ws,75,9,14)
    ...
    for c in range(9,17): ws.cell(83,c).fill=_fill(LIGHT)
    ws["I83"]="Monte Carlo shows the valuation distribution; scenarios/stress tests are deterministic; the DCF heatmap shows WACC and terminal-growth sensitivity."
    ws["I83"].alignment=Alignment(wrap_text=True,vertical="top")
