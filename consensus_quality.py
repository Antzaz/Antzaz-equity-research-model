...
def _derived(blended,metric,year):
    if metric=="EBIT Margin":
        ...
        if rev and ebit and _num(rev.get("mean")) not in (None,0) and _num(ebit.get("mean")) is not None:
            rm=_num(rev["mean"]); em=_num(ebit["mean"]); lo=None; hi=None
...
def _eps_revisions(result,year):
    ...
    return ((cur/d30-1) if cur not in (None,0) and d30 not in (None,0) else None),((cur/d90-1) if cur not in (None,0) and d90 not in (None,0) else None)
...
def _write_expectations(wb,result):
    ...
    for (metric,year),r in rows.items():
        ...
        ws.cell(r,3,item["mean"]); fmt=FMT_PCT if metric in {"EBIT Margin","Capex / Revenue"} else FMT_PRICE if metric=="EPS" else FMT_BN; ws.cell(r,3).number_format=fmt
        ws.cell(r,9,_range_dispersion(item)); ws.cell(r,9).number_format=FMT_PCT; ws.cell(r,12,item.get("provider_count")); ws.cell(r,12).number_format=FMT_INT; ws.cell(r,13,item.get("providers")); ws.cell(r,13).alignment=Alignment(wrap_text=True,vertical="top")
        ws.cell(r,11,f"Multi-source reference — {item.get('providers')}. Provider-level observations below; no analyst-count summation across aggregators."); ws.cell(r,11).alignment=Alignment(wrap_text=True,vertical="top")
        if metric=="EPS":
            r30,r90=_eps_revisions(result,year); ws.cell(r,7,r30); ws.cell(r,8,r90); ws.cell(r,7).number_format=FMT_PCT; ws.cell(r,8).number_format=FMT_PCT
        updated+=1
    start=ws.max_row+2; _section(ws,start,"Consensus Provider Status & API Coverage",13); _header(ws,start+1,["Provider","Configured","Usable Now","Detail","Source / Documentation"])
    ...
    prov=start+len(result["statuses"])+4; _section(ws,prov,"Consensus Source Cross-Check — Provider-Level Observations",13); _header(ws,prov+1,["Fiscal Year","Metric","Provider","Mean","Low","High","Analysts","Provider Spread","Fetched At","Source URL","Notes"])
    for rr,rec in enumerate(sorted(result["observations"],key=lambda x:(x.get("fiscal_year") or 0,x.get("metric") or "",x.get("provider") or "")),prov+2):
        ...
        mean=_num(rec.get("mean")); low=_num(rec.get("low")); high=_num(rec.get("high")); ws.cell(rr,8,((high-low)/abs(mean)) if mean not in (None,0) and low is not None and high is not None else None); ws.cell(rr,8).number_format=FMT_PCT
...
def _write_market_expectations(wb,result):
    ...
    ws=wb["Market Expectations"]; start=ws.max_row+2; _section(ws,start,"External Consensus Cross-Check — Street Estimates vs Market-Implied Hurdles",12); _header(ws,start+1,["Metric","Fiscal Year / Horizon","External Consensus","Your Model / Market Hurdle","Gap","Provider Count","Providers","Interpretation"])
    exp=wb["Expectations & Consensus"] if "Expectations & Consensus" in wb.sheetnames else None; rr=start+2
    if exp:
        rows=_row_map(exp); years=sorted({y for _,y in rows})[:3]
        for year in years:
            for metric in ("Revenue","EPS","EBIT Margin","FCF","Capex / Revenue"):
                ...
                for c,v in enumerate(vals,1): ws.cell(rr,c,v)
                fmt=FMT_PCT if metric in {"EBIT Margin","Capex / Revenue"} else FMT_PRICE if metric=="EPS" else FMT_BN; ws.cell(rr,3).number_format=fmt; ws.cell(rr,4).number_format=fmt; ws.cell(rr,5).number_format=FMT_PCT
    ...
    ws.column_dimensions["G"].width=max(ws.column_dimensions["G"].width or 0,34); ws.column_dimensions["H"].width=max(ws.column_dimensions["H"].width or 0,58); return True


def _write_quality(wb,result):
    if "Data Quality" not in wb.sheetnames: return
    ws=wb["Data Quality"]; good=[s for s in result["statuses"] if s.get("ok")]; configured=[s for s in result["statuses"] if s.get("configured")]; status="PASS" if good else "REVIEW"; detail=f"{len(good)}/{len(result['statuses'])} provider(s) returned usable consensus; {len(configured)} configured. Yahoo is zero-config; optional API sources improve coverage."
    existing=None
    for r in range(1,ws.max_row+1):
        if str(ws.cell(r,1).value or "").strip()=="Consensus provider coverage": existing=r; break
    r=existing or ws.max_row+1; ws.cell(r,1,"Consensus provider coverage"); ws.cell(r,2,status); ws.cell(r,3,detail); ws.cell(r,4,"Do not interpret several aggregators as independent analyst samples; compare provider levels/ranges and preserve provenance."); ws.cell(r,2).fill=_fill(PALE_GREEN if status=="PASS" else GOLD); ws.cell(r,2).font=Font(bold=True)
    for c in range(1,5): ws.cell(r,c).alignment=Alignment(wrap_text=True,vertical="top")


def _install_institutional_extension_hook():
    """Extend the institutional sheet immediately after research_extensions creates it.

    update_model imports research_extensions before this function runs. Replacing the module
    global is therefore a lightweight pipeline hook that avoids another duplicate generator.
    """
    try:
        import research_extensions
        from professional_analysis import extend_institutional_comparison
        original=research_extensions.ensure_institutional_lens_comparison
        if getattr(original,"_professional_extension",False): return
        def wrapped(wb,ticker):
            result=original(wb,ticker); extend_institutional_comparison(wb); return result
        wrapped._professional_extension=True
        research_extensions.ensure_institutional_lens_comparison=wrapped
    except Exception:
        pass


def normalize_expectations_consensus(wb,ticker,info=None):
    latest=_latest_year(wb); result=collect_consensus(ticker,latest,info or {}); exp=_write_expectations(wb,result); market=_write_market_expectations(wb,result); _write_quality(wb,result)
    try:
        from professional_analysis import extend_ai_impact_analysis
        extend_ai_impact_analysis(wb,ticker)
    except Exception as exc:
        print(f"Warning: professional AI methodology extension failed: {exc}")
    _install_institutional_extension_hook()
