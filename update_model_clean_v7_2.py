...
Usage:
    python update_model_clean_v7.py GOOGL
...
def put_history(wb,hist):
    # If SEC returns nothing, preserve the historical values already in the template.
    if not hist:
...
def _template_history_fallback(wb):
    """
    Read historical data already present in the Excel template.
    Used if SEC Company Facts returns no/insufficient annual history.
    """
    ws = wb["Historical Financials"]
    out = {}
    for c in range(2, 8):
...
def update_scenarios(wb,hist,info):
    ...
    # SEC data can occasionally be incomplete or unavailable.
    # Merge it with the template's existing historical data instead of crashing.
    fallback = _template_history_fallback(wb)
    merged = dict(fallback)
    merged.update(hist or {})
    years = sorted(y for y, d in merged.items() if d and d.get("revenue"))

    # Conservative defaults if fewer than two valid revenue years exist.
    ...
    if len(years) >= 2:
        last = years[-1]
        # Use up to a 5-year CAGR, depending on how much data is actually available.
        first = years[max(0, len(years)-6)]
        r0 = merged[first].get("revenue")
        r1 = merged[last].get("revenue")
        n = max(1, last-first)
        ...
        if rev:
            if latest.get("op") is not None:
                opm = latest["op"]/rev
            if latest.get("capex") is not None:
                cap = abs(latest["capex"])/rev
            if latest.get("depr") is not None:
                dep = abs(latest["depr"])/rev

    # Clamp obviously bad source values.
    ...
    growths = [
        fade(max(.03, base-.05), .04),
        fade(base, .06),
        fade(min(.22, base+.03), .08),
    ]

    blocks = [range(2,12), range(14,24), range(26,36)]
    for cols, gs in zip(blocks, growths):
        for c, v in zip(cols, gs):
            ws.cell(12, c).value = v
    ...
    for cols, ms in zip(blocks, margins):
        for c, v in zip(cols, ms):
            ws.cell(14, c).value = v

    capex_sets = [
        fade(min(.35, cap+.05), .16),
        fade(min(.35, cap+.01), .13),
        fade(max(.12, cap-.01), .12),
    ]
    for cols, cs in zip(blocks, capex_sets):
        for c, v in zip(cols, cs):
            ws.cell(20, c).value = v

    for cols in blocks:
        for c in cols:
            ws.cell(18, c).value = dep
...
def main():
    ticker=(sys.argv[1] if len(sys.argv)>1 else input("Ticker: ")).upper().strip()
    ...
    # Ensure Excel recalculates formulas when the output workbook is opened.
    # Some openpyxl/workbook combinations have wb.calculation = None.
    try:
