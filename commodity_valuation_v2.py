...
COMMODITY_POLICY={
    "default":{
        "bear_wacc_floor":.095,"base_wacc_floor":.080,"bull_wacc_floor":.0725,
        "bear_terminal":.010,"base_terminal":.015,"bull_terminal":.020,
        "reference_prices":"Bear/Base/Bull commodity normalization; issuer-specific price anchors when available",
    },
    "CVX":{
        "bear_wacc_floor":.095,"base_wacc_floor":.080,"bull_wacc_floor":.0725,
        "bear_terminal":.010,"base_terminal":.015,"bull_terminal":.020,
        "brent_bear":55.0,"brent_base":70.0,"brent_bull":85.0,
...
def _policy(ticker):
    base=dict(COMMODITY_POLICY["default"]); base.update(COMMODITY_POLICY.get(str(ticker).upper(),{})); return base


def _set_wacc_and_terminal(wb,ticker,meta):
    if "Three-Case Scenarios" not in wb.sheetnames: return meta
    ws=wb["Three-Case Scenarios"]; p=_policy(ticker); raw_base=_num(ws["C6"].value,.09)
    ...
    meta.update({"raw_calculated_wacc":raw_base,"bear_wacc":bear,"wacc":base,"bull_wacc":bull,
...
def _set_cvx_operating_path(wb,meta):
    ws=wb["Three-Case Scenarios"]; hist=legacy._historical_cycle(wb); p=_policy("CVX"); start=hist.get("latest_revenue")
    if not start: return meta
    bear_g=p["bear_growth"]; base_g=p["base_growth"]; bull_g=p["bull_growth"]; cycle=hist["margin"]
    ...
    for cols,(growths,margins,capex_rates) in zip(blocks,[(bear_g,bear_m,bear_cap),(base_g,base_m,base_cap),(bull_g,bull_m,bull_cap)]):
    ...
    meta.update({"hist_margin":cycle,"hist_da":hist["da"],"hist_capex":hist["capex"],"hist_years":hist["years"],
...
def _augment_proof(wb,ticker,meta):
    ...
    ws=wb["Commodity Valuation"]; p=meta.get("commodity_policy") or _policy(ticker)
    ws["A38"]="Commodity Risk / Price Normalization"
    for c,v in enumerate(["Assumption","Bear","Base","Bull","Unit","Purpose","Status","Comment"],1): ws.cell(39,c,v)
    rows=[
    ...
    ws["A46"]="Valuation Reconciliation"
    for c,v in enumerate(["Metric","Bear","Base","Bull","Current Price","Interpretation","Source","Status"],1): ws.cell(47,c,v)
    rows=[
    ...
    for r,row in enumerate(rows,48):
        for c,v in enumerate(row,1): ws.cell(r,c,v)
    for c in (2,3,4,5): ws.cell(48,c).number_format='$#,##0.00;[Red]($#,##0.00);-'
    for c in (2,3,4): ws.cell(49,c).number_format=FMT_PCT
...
def apply_commodity_normalization(wb,ticker,info=None):
    ...
    meta=_set_wacc_and_terminal(wb,ticker,meta); setattr(wb,"_commodity_valuation",meta); _augment_proof(wb,ticker,meta)
    print(f"Commodity valuation v2: {ticker}; base WACC={meta.get('wacc',0):.2%}; terminal={meta.get('terminal_growth',0):.2%}; raw CAPM/WACC={meta.get('raw_calculated_wacc',0):.2%}")
    return meta
